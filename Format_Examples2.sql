...
SELECT FORMAT(@dt2, 'dd/MM/yyyy', 'en-US') AS 'Date',
...
DROP FUNCTION IF EXISTS [dbo].[udfDateFormatV2]
GO

CREATE FUNCTION [dbo].[udfDateFormatV2](@date date, @dtype [tinyint])
RETURNS [nvarchar](25) 
AS 
BEGIN
    DECLARE @ret [nvarchar](25),
    ...
    SET @ret = 
        CASE @dtype
            WHEN 1 THEN ( SELECT FORMAT(@date, 'd', 'en-US') )
            WHEN 2 THEN ( SELECT FORMAT(@date, 'D', 'en-US') )
            WHEN 3 THEN ( SELECT FORMAT(@date, 'dd/MM/yyyy', 'en-US') )
            WHEN 4 THEN ( SELECT FORMAT(@date, 'yyyy-MM-dd', 'en-US') )
            WHEN 5 THEN ( SELECT FORMAT(@date, 'yyyy-MM', 'en-US') )
            WHEN 6 THEN ( SELECT FORMAT(@date, 'MM-yyy', 'en-US') )
            ELSE '** Invalid **'
        END;
...
DECLARE @dtype tinyint = 3,
		@dt		date = getdate()

SELECT	[dbo].[udfDateFormatV2](@dt, @dtype) as 'Date Format 1'
